	...
	public void updateBookingStatus() {
	    ...
	    try {
	        ...
	        int updatedCount = entityManager.createQuery(
	            "UPDATE Booking b SET b.status = 'inactive' WHERE b.status IN ('waiting', 'pending', 'active') AND b.bookingTime IS NOT NULL AND FUNCTION('TIMESTAMPDIFF', MINUTE, b.bookingTime, CURRENT_TIMESTAMP) >= 1")
	            .executeUpdate();

	        entityManager.getTransaction().commit();

	        System.out.println("Updated " + updatedCount + " bookings to inactive status.");
	    } catch (Exception ex) {
